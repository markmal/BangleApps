  ...
  var l = {
    ...
    draw : function(rowmin,rowmax) {
      ...
      while (rows--) {
        ...
        if (item.drawItem) item.drawItem(v, item, x,iy,x2,iy+options.fontHeight-1);
    ...
    select : function() {
      ...
      if ("function" == typeof item) needDraw = item(l);
        ...
        if ("number" == typeof item.value) {
          ...
          else if (item.onchange) needDraw = item.onchange(item.value);
        }
      }
      if(needDraw) l.draw(); //a func can return true if it needs the menu redraw
    },
    move : function(dir) {
